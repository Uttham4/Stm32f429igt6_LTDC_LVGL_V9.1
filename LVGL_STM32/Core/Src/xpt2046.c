...
#define TDIN1 HAL_GPIO_WritePin (TOUCH_MOSI_GPIO_Port,TOUCH_MOSI_Pin,GPIO_PIN_SET)
#define TDIN0 HAL_GPIO_WritePin (TOUCH_MOSI_GPIO_Port,TOUCH_MOSI_Pin,GPIO_PIN_RESET)

#define TCLK1 HAL_GPIO_WritePin (TOUCH_SCK_GPIO_Port,TOUCH_SCK_Pin,GPIO_PIN_SET)
#define TCLK0 HAL_GPIO_WritePin (TOUCH_SCK_GPIO_Port,TOUCH_SCK_Pin,GPIO_PIN_RESET)

#define TCS1 HAL_GPIO_WritePin (TOUCH_CS_GPIO_Port,TOUCH_CS_Pin,GPIO_PIN_SET)
#define TCS0 HAL_GPIO_WritePin (TOUCH_CS_GPIO_Port,TOUCH_CS_Pin,GPIO_PIN_RESET)
...
{ 	 
	...
	{ 				  
		...
		delayUs(20);    
 		TCLK1;
 		if(HAL_GPIO_ReadPin (TOUCH_MISO_GPIO_Port , TOUCH_MISO_Pin))
                {
                  Num++;
                }
	}  	
...
//   *y=raw_y;
    ...
    *y = XPT2046_SCALE_Y- (raw_y - XPT2046_MIN_RAW_Y) * XPT2046_SCALE_Y / (XPT2046_MAX_RAW_Y - XPT2046_MIN_RAW_Y);
		buff_len=sprintf(buff, "RX = %d, RY = %d, X=%d,Y=%d\r\n", (int) raw_x, (int) raw_y, *x, *y);
